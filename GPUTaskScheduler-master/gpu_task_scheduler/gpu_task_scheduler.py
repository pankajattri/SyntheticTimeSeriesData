...
def gpu_worker(name, env, _lock, _config, _gpu_task_class, _config_manager,
    ...
    while True:
        ...
        module_path = os.path.abspath(
            sys.modules[_gpu_task_class.__module__].__file__)
        if module_path[-3:] == "pyc":
            module_path = module_path[:-1]

        module_name = _gpu_task_class.__module__

        cmd = "start_gpu_task \"{}\" \"{}\" \"{}\" \"{}\"".format(
            pkl_path, module_name, module_path, os.getcwd())
        args = shlex.split(cmd)
        p = subprocess.Popen(args, env=new_env, stdout=Unbuffered(
            log_file), stderr=subprocess.STDOUT, cwd=os.getcwd())
        p.wait()
